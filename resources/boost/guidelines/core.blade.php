...
- This package owns `Models\{Plan,Subscription,SubscriptionPayment}`, `Enums\{PeriodUnit,SubscriptionStatus,SubscriptionPaymentStatus}`, `Exceptions\{PlanInUseException,SubscriptionPaymentException}`, `Database\Seeders\PlanSeeder`, and `SubscriptionServiceProvider`. `PlanSeeder` ships the default Free/Basic/Pro catalog and seeds it idempotently by slug; the console panel owns plan CRUD from there. Depend only on `misaf/vendra-support` (for `ShouldLogActivity`) and framework/Spatie packages — never on a domain provider such as `misaf/vendra-tenant`, `misaf/vendra-user`, or `misaf/vendra-permission`.
