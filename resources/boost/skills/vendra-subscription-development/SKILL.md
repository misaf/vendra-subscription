...
- The concrete subscriber (`App\Models\Reseller`, which implements `SubscriptionSubscriber`), quota (`PropertyQuota`), provisioning (`Create/ProvisionTenantAction`), owner notifications, the host reaction listeners (`NotifyActivatedSubscriber`, `RemindExpiringSubscriber`, `SuspendSubscriberProperties`), and the concrete `SubscriptionCharger` binding all live in the **host app** (`app/…`), not here. Console/reseller Filament panels are app-level (`app/Filament/{Console,Reseller}`).
