...
- Operator cancellation, extension, and reactivation use `CancelSubscriptionAction`, `ExtendSubscriptionAction`, and `ReactivateSubscriptionAction`. Cancellation emits `SubscriptionCancelled` after commit; reactivation, renewal, and plan changes converge on `SubscribeAction`. Keep subscriber-specific reactions outside this package.
