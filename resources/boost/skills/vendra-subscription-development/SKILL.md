...
- Use namespace `Misaf\VendraSubscription`. Own `Models\{Plan,Subscription,SubscriptionPayment}`, `Enums\{PeriodUnit,SubscriptionStatus,SubscriptionPaymentStatus}`, `Contracts\SubscriptionSubscriber`, `Exceptions\{PlanInUseException,SubscriptionPaymentException,SubscriptionLimitException}`, `Actions\{SubscribeAction,ActivateSubscriptionAction,CancelSubscriptionAction,ExtendSubscriptionAction,ReactivateSubscriptionAction,ChangeSubscriptionPlanAction,RenewSubscriptionAction,SetSubscriptionAutoRenewAction,ChargeSubscriptionAction,ApplySubscriptionPaymentResultAction,EnforceSubscriptionsAction}`, `Support\{PlanChangeQuote,MoneyFormatter}`, `Jobs\ProcessSubscriptionPayment`, `Console\Commands\{EnforceSubscriptionsCommand,RecoverSubscriptionPaymentsCommand,ReportSubscriptionPaymentBacklogCommand}`, `Events\{SubscriptionPaymentPaid,SubscriptionPaymentFailed,SubscriptionActivated,SubscriptionCancelled,SubscriptionExpiringSoon,SubscriptionGraceExpired}`, `Listeners\ActivateSubscriptionOnPayment`, `Database\Seeders\PlanSeeder`, and `SubscriptionServiceProvider`. `PlanSeeder` ships the package's default Free/Basic/Pro catalog, seeded idempotently by slug; plans are edited from the console panel afterwards.
...
- **Subscriber decoupling:** `Subscription::subscriber()` is a generic `MorphTo` over `subscriber_type` / `subscriber_id`; never import a concrete subscriber here. `SubscriptionSubscriber` declares the subscription lookup, payer, notification, and unit-count reads the engine needs as domain methods, NOT the raw Eloquent relation — an Eloquent relation's declaring-model generic is invariant and cannot sit in an interface at phpstan level 10. The host subscriber model implements the contract and defines the inverse `morphMany`, registers stable morph aliases, and migrates pre-alias FQCN values. `SubscriptionFactory::forSubscriber($model)` stores the model's morph class, while a bare ID uses the explicit generic default type. The `(subscriber_type, active_subscriber_guard)` unique index enforces one active subscription per subscriber.
- **Event boundary:** the engine performs generic state changes and raises events; the host reacts. `ChargeSubscriptionAction` raises `SubscriptionPaymentPaid`; the package's `ActivateSubscriptionOnPayment` listener activates and raises `SubscriptionActivated`; `EnforceSubscriptionsAction` raises `SubscriptionExpiringSoon`/`SubscriptionGraceExpired`. Unit reactivation stays ATOMIC inside activation/subscribe through the `Contracts\SubscriptionUnitSuspender` binding (same transaction; `Support\NullSubscriptionUnitSuspender` by default, so subscriber models never run another package's actions); only notifications and tenant suspension are event reactions. Register package listeners explicitly in `packageBooted()`; host `app/Listeners` are auto-discovered (Laravel 11+) — never also `Event::listen` them or reactions double-fire.
...
- Cancellation, extension, and reactivation use `CancelSubscriptionAction`, `ExtendSubscriptionAction`, and `ReactivateSubscriptionAction`. Eligibility comes only from `Subscription::canBeCancelled()`, `canBeReactivated()`, and `canBeExtended()`, which the actions enforce under a row lock and the console uses for visibility. The cancellable statuses live in `SubscriptionStatus::cancellable()` and the payments still being collected in the `SubscriptionPayment::open()` scope; never repeat either list inline. Cancellation emits `SubscriptionCancelled` after commit; reactivation, renewal, and plan changes converge on `SubscribeAction`. Keep subscriber-specific reactions outside this package.
...
- Plan changes use `ChangeSubscriptionPlanAction`: an upgrade applies now (keeping a paid running period's `ends_at` and collecting the prorated difference), a downgrade is scheduled in `scheduled_plan_id` for `RenewSubscriptionAction`. `Support\PlanChangeQuote` is the only proration rule; panels read it rather than recomputing. Renewal only runs for a period that is no longer active and continues from its `ends_at` while its grace window is open. `EnforceSubscriptionsAction` renews lapsed `auto_renews` periods before expiring them and measures grace from the latest `activated()` period (`activated_at`), so an unpaid renewal cannot postpone suspension.
